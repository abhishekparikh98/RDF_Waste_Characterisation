...
The 60-to-6 TACO category mapping is documented inline in the preprocessing scripts under `scripts/` (see `scripts/convert_taco_to_yolo.py` lines 64–86 for `TACO_CATEGORY_TO_PROJECT`, and `data/yolo/data.yaml` for the 6-class names used during training). See also `docs/TACO_TO_YOLO_MAPPING.md` for a quick reference.
...
On a fresh clone the model weights (`models/yolo_best.pt` and
`models/cnn_baseline_best.h5`) are not in the repo. Two options:

- **Auto-download (default)**: `app.py` downloads both from the GitHub
  Release tagged `v1.0-models` on first request. If the Release has not
  yet been published, you will see a clear error message — either create
  the Release and attach the two files, or fall back to the manual step
  below.
- **Manual**: place the trained weights at `models/yolo_best.pt` and
  `models/cnn_baseline_best.h5` before starting the server, or retrain
  following the [Reproducing the Trained Models](#-reproducing-the-trained-models)
  section.

`models/rdf_random_forest_pipeline.joblib` **is** tracked in git, so the
RDF side works without any extra step.
